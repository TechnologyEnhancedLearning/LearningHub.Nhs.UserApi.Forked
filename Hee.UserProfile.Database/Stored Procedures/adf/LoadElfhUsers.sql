...
BEGIN
	...
	BEGIN TRY
		...
		-- Create new User/s
		INSERT INTO dbo.[User]([Id],[UserName],[EmailAddress],[FirstName],[LastName],[Active],[Deleted],[CreateUserId],[CreateDate],[AmendUserId],[AmendDate])
		SELECT 
		...
		FROM
			adf.Staging_Elfh_User elfh_user
		LEFT JOIN
			dbo.[User] u ON elfh_user.userId = u.Id
		WHERE
			JobRunId = @JobRunId
			AND u.Id IS NULL
